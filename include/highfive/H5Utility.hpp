...
class SilenceHDF5 {
public:
  inline SilenceHDF5(bool enable = true) : _client_data(nullptr) {
...
public:
  ...
  explicit Logger(callback_type cb) : _cb(std::move(cb)) {}
...
inline void default_logging_callback(LogSeverity severity,
                                     ...
                                     const std::string &file, int line) {
  std::clog << file << ": " << line << " [" << to_string(severity) << "] "
            << message << std::endl;
}
...
#define HIGHFIVE_LOG_DEBUG(message)                                            \
  ::HighFive::detail::log(::HighFive::LogSeverity::Debug, (message), __FILE__, \
                          __LINE__);
...
#define HIGHFIVE_LOG_INFO(message)                                             \
  ::HighFive::detail::log(::HighFive::LogSeverity::Info, (message), __FILE__,  \
                          __LINE__);
...
#define HIGHFIVE_LOG_WARN(message)                                             \
  ::HighFive::detail::log(::HighFive::LogSeverity::Warn, (message), __FILE__,  \
                          __LINE__);
...
#define HIGHFIVE_LOG_ERROR(message)                                            \
  ::HighFive::detail::log(::HighFive::LogSeverity::Error, (message), __FILE__, \
                          __LINE__);
