...
public:
  ...
  explicit File(
      const std::string &filename, AccessMode openFlags = ReadOnly,
  ...
  ///
  /// Open or create a new HDF5 file
  File(const std::string &filename, AccessMode openFlags,
...
inline File::AccessMode operator|(File::AccessMode lhs, File::AccessMode rhs) {
  ...
  return static_cast<File::AccessMode>(static_cast<int_t>(lhs) |
                                       static_cast<int_t>(rhs));
}
...
inline const File::AccessMode &operator|=(File::AccessMode &lhs,
                                          File::AccessMode rhs) {
  lhs = lhs | rhs;
  return lhs;
}

inline File::AccessMode operator&=(File::AccessMode &lhs,
                                   File::AccessMode rhs) {
  lhs = lhs & rhs;
  return lhs;
}
