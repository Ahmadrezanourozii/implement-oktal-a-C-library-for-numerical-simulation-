...
template <typename T> struct inspector<boost::numeric::ublas::matrix<T>> {
  ...
  static void prepare(type &val, const std::vector<size_t> &dims) {
    if (dims.size() < ndim) {
      ...
      os << "Impossible to pair DataSet with " << dims.size()
         << " dimensions into a " << ndim << " boost::numeric::ublas::matrix";
  ...
  static void serialize(const type &val, const std::vector<size_t> &dims,
    ...
    for (size_t i = 0; i < size; ++i) {
      inspector<value_type>::serialize(*(&val(0, 0) + i), subdims,
                                       m + i * subsize);
    }
  }
