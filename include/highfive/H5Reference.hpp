...
///
/// HDF5 object references allow pointing to groups, datasets (and compound
/// types). They differ from links in their ability to be stored and retrieved
/// as data from the HDF5 file in datasets themselves.
///
class Reference {
public:
  ...
  ///
  /// \param location A File or Group where the object being referenced to
  /// resides
  ...
  /// \param location the location where the referenced object is to be found (a
  /// File)
  /// \return the dereferenced Object (either a Group or DataSet)
  template <typename T> T dereference(const Object &location) const;
...
protected:
  ...
  inline explicit Reference(const hobj_ref_t h5_ref) : href(h5_ref) {};

  /// \brief Create the low-level reference and store it at refptr
  ///
  /// \param refptr Pointer to a memory location where the created HDF5
