...
namespace details {
template <typename T, typename U> T get_plist(const U &obj, hid_t (*f)(hid_t)) {
...
///
/// \sa Instructions to document C++20 concepts with Doxygen:
/// https://github.com/doxygen/doxygen/issues/2732#issuecomment-509629967
///
/// \cond
...
///
template <PropertyType T> class PropertyList : public PropertyListBase {
public:
  ///
  ...
  template <PropertyInterface P> void add(const P &property);
...
template <PropertyType T> class RawPropertyList : public PropertyList<T> {
public:
...
///
/// Note that when used in a file access property list, this will force all
/// reads of meta data to be collective. HDF5 function may implicitly perform
/// metadata reads. These functions would become collective. A list of functions
/// that perform metadata reads can be found in the HDF5 documentation, e.g.
///    https://docs.hdfgroup.org/hdf5/v1_12/group___g_a_c_p_l.html
///
/// In HighFive setting collective read is (currently) only supported on file
/// level.
///
/// Please also consult upstream documentation of
...
///
/// See the upstream documentation of `H5Pget_file_space_strategy` for more
/// details. Essentially, it enables configuring how space is allocate in the
/// file.
///
...
public:
  ...
  /// \param persist Should free space managers be persisted across file closing
  /// and reopening.
  /// \param threshold The free-space manager wont track sections small than
  /// this threshold.
  FileSpaceStrategy(H5F_fspace_strategy_t strategy, hbool_t persist,
                    hsize_t threshold);
...
///
/// See the upstream documentation of `H5Pset_file_space_page_size` for more
/// details. Essentially, it enables configuring the page size when paged
/// allocation is used.
///
/// General information about paged allocation can be found in the upstream
/// documentation "RFC: Page Buffering".
///
...
public:
  ...
  /// @param min_meta_percent fraction of the page buffer dedicated to meta
  /// data, in percent.
  /// @param min_raw_percent fraction of the page buffer dedicated to raw data,
  /// in percent.
...
public:
  ...
  template <typename... Args> explicit Chunking(hsize_t item, Args... args);
...
///
/// The cause refers to the most recent I/O with data transfer property list
/// `dxpl` at time of creation of this object. This object will not update
/// automatically for later data transfers, i.e.
/// `H5Pget_mpio_no_collective_cause` is called in the constructor, and not when
/// fetching a value, such as `wasCollective`.
/// \implements PropertyInterface
...
public:
  ...
  ///
  explicit LinkCreationOrder(unsigned flags) : _flags(flags) {}
