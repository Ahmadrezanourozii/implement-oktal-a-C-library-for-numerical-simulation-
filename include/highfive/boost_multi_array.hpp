...
struct inspector<boost::multi_array<T, Dims>> {
  ...
  static void prepare(type &val, const std::vector<size_t> &dims) {
    ...
    std::size_t size =
        std::accumulate(dims.begin(), dims.begin() + Dims, std::size_t{1},
  ...
  static void serialize(const type &val, const std::vector<size_t> &dims,
    ...
    for (size_t i = 0; i < size; ++i) {
      inspector<value_type>::serialize(*(val.origin() + i), subdims,
                                       m + i * subsize);
    }
  }

  template <class It>
  static void unserialize(It vec_align, const std::vector<size_t> &dims,
                          type &val) {
    assert_c_order(val);
    ...
    for (size_t i = 0; i < val.num_elements(); ++i) {
      inspector<value_type>::unserialize(vec_align + i * subsize, next_dims,
                                         *(val.origin() + i));
    }
  }
};
