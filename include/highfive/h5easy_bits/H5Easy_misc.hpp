...
inline Exception error(const File &file, const std::string &path,
...
inline Exception dump_error(File &file, const std::string &path) {
  if (file.getObjectType(path) == ObjectType::Dataset) {
    ...
    return error(file, path,
                 "H5Easy: path exists, but does not correspond to a Dataset. "
                 "Dump not possible.");
  }
}
...
template <class T>
inline DataSet initDataset(File &file, const std::string &path,
  ...
  if (!file.exist(path)) {
    if (!options.compress() && !options.isChunked()) {
      ...
      return file.createDataSet<T>(path, DataSpace(shape), props, {}, true);
             ...
             file.getObjectType(path) == ObjectType::Dataset) {
...
template <class T>
inline DataSet initScalarDataset(File &file, const std::string &path,
                                 const T &data, const DumpOptions &options) {
  if (!file.exist(path)) {
...
template <class T>
inline Attribute
initAttribute(File &file, const std::string &path, const std::string &key,
              const std::vector<size_t> &shape, const DumpOptions &options) {
  if (!file.exist(path)) {
...
template <class T>
inline Attribute initScalarAttribute(File &file, const std::string &path,
