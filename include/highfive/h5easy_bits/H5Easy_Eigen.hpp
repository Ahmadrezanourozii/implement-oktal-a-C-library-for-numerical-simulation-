...
template <typename T>
struct io_impl<T, typename std::enable_if<
                      std::is_base_of<Eigen::DenseBase<T>, T>::value>::type> {
  ...
  mem_shape(const File &file, const std::string &path, const D &dataset) {
  ...
  inline static Attribute dumpAttribute(File &file, const std::string &path,
    ...
    std::vector<size_t> mem_dims = mem_shape(data);
    Attribute attribute =
        initAttribute<value_type>(file, path, key, file_dims, options);
  ...
  inline static T loadAttribute(const File &file, const std::string &path,
