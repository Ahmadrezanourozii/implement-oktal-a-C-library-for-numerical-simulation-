...
/*
Base template for partial specialization: the fallback if specialized templates
don't match. Used e.g. for scalars.
*/
template <typename T, typename = void>
struct io_impl : public default_io_impl<T> {
  inline static DataSet dump_extend(File &file, const std::string &path,
                                    const T &data,
    ...
    if (file.exist(path)) {
      ...
      if (dims.size() != idx.size()) {
        throw detail::error(file, path,
                            "H5Easy::dump: Dimension of the index and the "
    ...
    DataSet dataset =
        file.createDataSet(path, dataspace, AtomicType<T>(), props, {}, true);
    dataset.select(idx, ones).write(data);
    if (options.flush()) {
      file.flush();
    }
  ...
  inline static T load_part(const File &file, const std::string &path,
                            const std::vector<size_t> &idx) {
    std::vector<size_t> ones(idx.size(), 1);
