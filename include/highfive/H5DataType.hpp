...
public:
  ...
  ///
  /// Notice that the size of variable length sequences may have limited
  /// applicability given that it refers to the size of the control structure.
  /// For info see
...
public:
  ...
  /// with 0 or more `\0`.
  FixedLengthStringType(size_t size, StringPadding padding,
...
///
template <typename T> class AtomicType : public DataType {
public:
  AtomicType();
...
public:
  ...
  struct member_def {
    member_def(std::string t_name, DataType t_base_type, size_t t_offset = 0)
        : name(std::move(t_name)), base_type(std::move(t_base_type)),
          offset(t_offset) {}
    std::string name;
    DataType base_type;
    size_t offset;
  ...
  /// \param type
  inline CompoundType(DataType &&type) : DataType(type) {
...
public:
  ...
  EnumType(const std::vector<member_def> &t_members) : members(t_members) {
...
template <typename T> DataType create_datatype();

/// \brief Create a DataType instance representing type T and perform a sanity
/// check on its size
template <typename T> DataType create_and_check_datatype();
} // namespace HighFive
