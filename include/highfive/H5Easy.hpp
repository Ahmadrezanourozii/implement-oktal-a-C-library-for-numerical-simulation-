...
enum class DumpMode {
  ...
  Overwrite = 1 /*!< Create or overwrite if DataSet of correct shape exists,
                   otherwise throw. */
};
...
public:
  ...
  template <class T> Compression(T level);
...
public:
  ...
  /// \param args any of DumpMode(), Flush(), Compression() in arbitrary number
  /// and order.
  template <class... Args> DumpOptions(Args... args) { set(args...); }
  ...
  /// \param arg any of DumpMode(), Flush(), Compression in arbitrary number and
  /// order.
  /// \param args any of DumpMode(), Flush(), Compression in arbitrary number
  /// and order.
  template <class T, class... Args> inline void set(T arg, Args... args);

  ///
  /// \brief Set chunk-size. If the input is rank (size) zero, automatic
  /// chunking is enabled.
  /// \param shape Chunk size along each dimension.
  template <class T> inline void setChunkSize(const std::vector<T> &shape);
  ...
  ///
  /// \brief Get chunking mode: ``true`` is manually set, ``false`` if
  /// chunk-size should be computed automatically.
  /// \return bool
  inline bool isChunked() const;

  ///
  /// \brief Get chunk size. Use DumpOptions::getChunkSize to check if
  /// chunk-size should be automatically computed.
...
template <class T>
inline DataSet dump(File &file, const std::string &path, const T &data,
                    const std::vector<size_t> &idx, const DumpOptions &options);
...
///
/// \brief Load entry ``{i, j, ...}`` from a DataSet in an open HDF5 file to a
/// scalar.
///
/// \param file opened file (has to be writeable)
...
/// \return The read data
///
template <class T> inline T load(const File &file, const std::string &path);
...
///
template <class T>
inline Attribute dumpAttribute(File &file, const std::string &path,
...
///
template <class T>
inline T loadAttribute(const File &file, const std::string &path,
