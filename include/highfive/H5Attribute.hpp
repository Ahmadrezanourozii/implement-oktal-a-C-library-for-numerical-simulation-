...
///
/// WARNING: Creating an Attribute from an ID has implications w.r.t. the
/// lifetime of the object
///          that got passed via its ID. Using this method careless opens up the
///          suite of issues related to C-style resource management, including
///          the analog of double free, dangling pointers, etc.
///
/// NOTE: This is not part of the API and only serves to work around a compiler
/// issue in GCC which
///       prevents us from using `friend`s instead. This function should only be
///       used for internal purposes. The problematic construct is:
///
...
///
/// \sa AnnotateTraits::createAttribute, AnnotateTraits::getAttribute,
/// AnnotateTraits::listAttributeNames, AnnotateTraits::hasAttribute,
/// AnnotateTraits::deleteAttribute for create, get, list, check or delete
/// Attribute
...
public:
  ...
  /// \code{.cpp}
  /// auto attr = dset.createAttribute<std::string>("my_attribute",
  /// DataSpace::From(string_list)); std::cout << attr.getName() << std::endl;
  /// // Will print "my_attribute"
  /// \endcode
  /// \since 2.2.2
  std::string getName() const;

  /// \brief The number of bytes required to store the attribute in the HDF5
  /// file.
  /// \code{.cpp}
  /// size_t size = dset.createAttribute<int>("foo", DataSpace(1,
  /// 2)).getStorageSize();
  /// \endcode
  /// \since 1.0
  ...
  /// \code{.cpp}
  /// Attribute attr = dset.createAttribute<int>("foo", DataSpace(1, 2));
  /// auto dspace = attr.getSpace(); // This will be a DataSpace of dimension 1
  /// * 2
  /// \endcode
  /// \since 1.0
  ...
  /// \endcode
  /// \since 1.0
  template <typename T> void read(T &array) const;

  /// \brief Read the attribute into a pre-allocated buffer.
  /// \param array A pointer to the first byte of sufficient pre-allocated
  /// memory.
  ...
  ///
  /// \note If possible prefer either Attribute::read() const or
  /// Attribute::read(T&) const.
  ///
  /// \code{.cpp}
  /// auto attr = file.getAttribute("foo");
  ///
  ...
  /// \endcode
  /// \since 2.2.2
  template <typename T> void read_raw(T *array) const;
  ...
  /// \endcode
  /// \since 2.2.2
  template <typename T> void write_raw(const T *buffer);
...
namespace detail {
inline Attribute make_attribute(hid_t hid) { return Attribute(hid); }
} // namespace detail
