...
namespace detail {
/// @brief Create a HighFive::DataSpace from an HID, without incrementing the
/// id.
///
...
public:
  ...
  ///
  /// This value should be used with DataSpace::DataSpace(const
  /// std::vector<size_t>& dims, const std::vector<size_t>& maxdims);
  ///
  /// \since 2.0
  static const size_t UNLIMITED = SIZE_MAX;

  /// \brief An enum to create scalar and null DataSpace with
  /// DataSpace::DataSpace(DataspaceType dtype).
  ///
  /// This enum is needed otherwise we will not be able to distringuish between
  /// both with normal constructors. Both have a dimension of 0.
  /// \since 1.3
  ...
  constexpr static DataspaceType dataspace_scalar =
      DataspaceType::dataspace_scalar;
  ...
  /// \endcode
  /// \since 2.3
  template <size_t N> explicit DataSpace(const std::array<size_t, N> &dims);
  ...
  /// \endcode
  /// \since 2.1
  template <typename... Args> explicit DataSpace(size_t dim1, Args... dims);
  ...
  template <typename IT, typename = typename std::enable_if<
                             !std::is_integral<IT>::value, IT>::type>
  ...
  ///
  /// \code{.cpp}
  /// DataSpace dspace(std::vector<size_t>{1, 3}, std::vector<size_t>{UNLIMITED,
  /// 10}); dspace.getMaxDimensions(); // Return {UNLIMITED, 10}
  /// \endcode
  ///
  ...
  /// auto dspace = DataSpace::FromCharArrayStrings(string_array); // dspace is
  /// a DataSpace of dimensions 2
  /// \endcode
  /// \since 2.2
