...
inline herr_t h5p_set_file_space_strategy(hid_t plist_id,
  ...
  herr_t err =
      H5Pset_file_space_strategy(plist_id, strategy, persist, threshold);
  if (err < 0) {
    HDF5ErrMapper::ToException<PropertyException>(
...
inline herr_t h5p_get_file_space_strategy(hid_t plist_id,
                                          ...
                                          hsize_t *threshold) {
  herr_t err =
      H5Pget_file_space_strategy(plist_id, strategy, persist, threshold);
  if (err) {
    HDF5ErrMapper::ToException<PropertyException>(
...
inline herr_t h5p_get_page_buffer_size(hid_t plist_id, size_t *buf_size,
                                       unsigned *min_meta_perc,
                                       unsigned *min_raw_perc) {
  herr_t err =
      H5Pget_page_buffer_size(plist_id, buf_size, min_meta_perc, min_raw_perc);
...
inline herr_t h5p_set_page_buffer_size(hid_t plist_id, size_t buf_size,
                                       unsigned min_meta_per,
                                       unsigned min_raw_per) {
  herr_t err =
      H5Pset_page_buffer_size(plist_id, buf_size, min_meta_per, min_raw_per);
  if (err < 0) {
    HDF5ErrMapper::ToException<PropertyException>(
        "Error setting page buffer size.");
  }
...
inline herr_t h5p_set_all_coll_metadata_ops(hid_t plist_id,
...
inline herr_t h5p_get_all_coll_metadata_ops(hid_t plist_id,
                                            hbool_t *is_collective) {
...
inline herr_t h5p_set_coll_metadata_write(hid_t plist_id,
                                          hbool_t is_collective) {
...
inline herr_t h5p_get_coll_metadata_write(hid_t plist_id,
                                          hbool_t *is_collective) {
...
inline herr_t h5p_get_libver_bounds(hid_t plist_id, H5F_libver_t *low,
                                    H5F_libver_t *high) {
...
inline herr_t h5p_set_libver_bounds(hid_t plist_id, H5F_libver_t low,
                                    H5F_libver_t high) {
...
inline herr_t h5p_set_est_link_info(hid_t plist_id, unsigned est_num_entries,
...
inline herr_t h5p_get_est_link_info(hid_t plist_id, unsigned *est_num_entries,
                                    unsigned *est_name_len) {
...
inline herr_t h5p_set_szip(hid_t plist_id, unsigned options_mask,
...
inline herr_t h5p_get_chunk_cache(hid_t dapl_id, size_t *rdcc_nslots,
...
inline herr_t h5p_set_chunk_cache(hid_t dapl_id, size_t rdcc_nslots,
                                  size_t rdcc_nbytes, double rdcc_w0) {
...
inline herr_t h5p_set_create_intermediate_group(hid_t plist_id,
                                                unsigned crt_intmd) {
...
inline herr_t h5p_get_create_intermediate_group(hid_t plist_id,
                                                unsigned *crt_intmd) {
...
inline herr_t
h5p_get_mpio_no_collective_cause(hid_t plist_id,
                                 uint32_t *local_no_collective_cause,
                                 uint32_t *global_no_collective_cause) {
  herr_t err = H5Pget_mpio_no_collective_cause(
...
inline herr_t h5p_set_link_creation_order(hid_t plist_id,
...
inline herr_t h5p_get_link_creation_order(hid_t plist_id,
                                          unsigned *crt_order_flags) {
...
inline herr_t h5p_get_attr_phase_change(hid_t plist_id, unsigned *max_compact,
                                        unsigned *min_dense) {
...
inline herr_t h5p_set_attr_phase_change(hid_t plist_id, unsigned max_compact,
                                        unsigned min_dense) {
