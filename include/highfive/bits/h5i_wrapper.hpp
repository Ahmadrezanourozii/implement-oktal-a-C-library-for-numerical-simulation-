...
inline int h5i_dec_ref(hid_t id) { return H5Idec_ref(id); }
...
namespace nothrow {
inline htri_t h5i_is_valid(hid_t id) { return H5Iis_valid(id); }
...
template <class Exception> inline hid_t h5i_get_file_id(hid_t id) {
  hid_t file_id = H5Iget_file_id(id);
  if (file_id < 0) {
    HDF5ErrMapper::ToException<Exception>(
