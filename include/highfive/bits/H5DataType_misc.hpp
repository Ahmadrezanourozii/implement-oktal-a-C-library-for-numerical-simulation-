...
inline bool DataType::empty() const noexcept { return _hid == H5I_INVALID_HID; }

inline DataTypeClass DataType::getClass() const {
  return convert_type_class(detail::h5t_get_class(_hid));
}

inline size_t DataType::getSize() const { return detail::h5t_get_size(_hid); }
...
inline FixedLengthStringType::FixedLengthStringType(
    size_t size, StringPadding padding, CharacterSet character_set) {
  if (size == 0 && padding == StringPadding::NullTerminated) {
    throw DataTypeException("Fixed-length, null-terminated need at least one "
...
inline VariableLengthStringType::VariableLengthStringType(
...
// char mapping
template <> inline AtomicType<char>::AtomicType() {
  _hid = detail::h5t_copy(H5T_NATIVE_CHAR);
}

template <> inline AtomicType<signed char>::AtomicType() {
  _hid = detail::h5t_copy(H5T_NATIVE_SCHAR);
}

template <> inline AtomicType<unsigned char>::AtomicType() {
  _hid = detail::h5t_copy(H5T_NATIVE_UCHAR);
}

// short mapping
template <> inline AtomicType<short>::AtomicType() {
  _hid = detail::h5t_copy(H5T_NATIVE_SHORT);
}

template <> inline AtomicType<unsigned short>::AtomicType() {
  _hid = detail::h5t_copy(H5T_NATIVE_USHORT);
}

// integer mapping
template <> inline AtomicType<int>::AtomicType() {
  _hid = detail::h5t_copy(H5T_NATIVE_INT);
}

template <> inline AtomicType<unsigned>::AtomicType() {
  _hid = detail::h5t_copy(H5T_NATIVE_UINT);
}

// long mapping
template <> inline AtomicType<long>::AtomicType() {
  _hid = detail::h5t_copy(H5T_NATIVE_LONG);
}

template <> inline AtomicType<unsigned long>::AtomicType() {
  _hid = detail::h5t_copy(H5T_NATIVE_ULONG);
}

// long long mapping
template <> inline AtomicType<long long>::AtomicType() {
  _hid = detail::h5t_copy(H5T_NATIVE_LLONG);
}

template <> inline AtomicType<unsigned long long>::AtomicType() {
  _hid = detail::h5t_copy(H5T_NATIVE_ULLONG);
}

// half-float, float, double and long double mapping
template <> inline AtomicType<float>::AtomicType() {
  _hid = detail::h5t_copy(H5T_NATIVE_FLOAT);
}

template <> inline AtomicType<double>::AtomicType() {
  _hid = detail::h5t_copy(H5T_NATIVE_DOUBLE);
}

template <> inline AtomicType<long double>::AtomicType() {
  _hid = detail::h5t_copy(H5T_NATIVE_LDOUBLE);
}

// std string
template <> inline AtomicType<std::string>::AtomicType() {
  _hid = create_string(H5T_VARIABLE);
}

#if HIGHFIVE_CXX_STD >= 17
// std byte
template <> inline AtomicType<std::byte>::AtomicType() {
...
template <size_t StrLen> class AtomicType<char[StrLen]> : public DataType {
public:
  inline AtomicType() : DataType(create_string(StrLen)) {}
};

template <typename T> class AtomicType<std::complex<T>> : public DataType {
public:
  inline AtomicType()
...
inline EnumType<details::Boolean> create_enum_boolean() {
  return {{"FALSE", details::Boolean::HighFiveFalse},
          {"TRUE", details::Boolean::HighFiveTrue}};
}

// Other cases not supported. Fail early with a user message
template <typename T> AtomicType<T>::AtomicType() {
  static_assert(true, "Missing specialization of AtomicType<T>. Therefore, "
                      "type T is not supported by HighFive.");
}

// Internal
// Reference mapping
template <> inline AtomicType<Reference>::AtomicType() {
...
// For padding see explanation here:
// https://en.cppreference.com/w/cpp/language/object#Alignment It is to compute
// padding following last element inserted inside a struct 1) We want to push
// back an element padded to the structure 'current_size' is the size of the
// structure before adding the new element. 'member_size' the size of the
// element we want to add. 2) We want to compute the final padding for the
// global structure 'current_size' is the size of the whole structure without
// final padding 'member_size' is the maximum size of all element of the struct
//
// The basic formula is only to know how much we need to add to 'current_size'
// to fit 'member_size'. And at the end, we do another computation because the
// end padding, should fit the biggest element of the struct.
//
...
#define _H5_STRUCT_PADDING(current_size, member_size)                          \
  (((member_size) >= (current_size))                                           \
       ...
       : ((((member_size) -                                                    \
            (((current_size) - (member_size)) % (member_size)))) %             \
          (member_size)))
...
inline void CompoundType::create(size_t size) {
  if (size == 0) {
    ...
    for (auto &member : members) {
      ...
      size_t first_atomic_size =
          find_first_atomic_member_size(member.base_type.getId());

      // Set the offset of this member within the struct according to the
      // standard alignment rules. The c++ standard specifies that:
      // > objects have an alignment requirement of which their size is a
      // multiple
      member.offset =
          current_size + _H5_STRUCT_PADDING(current_size, first_atomic_size);
  ...
  for (const auto &member : members) {
    detail::h5t_insert(_hid, member.name.c_str(), member.offset,
                       member.base_type.getId());
  }
}

#undef _H5_STRUCT_PADDING

inline void CompoundType::commit(const Object &object,
                                 const std::string &name) const {
  detail::h5t_commit2(object.getId(), name.c_str(), getId(), H5P_DEFAULT,
                      H5P_DEFAULT, H5P_DEFAULT);
}

template <typename T> inline void EnumType<T>::create() {
  // Create the HDF5 type
  _hid = detail::h5t_enum_create(
      AtomicType<typename std::underlying_type<T>::type>{}.getId());
...
template <typename T>
inline void EnumType<T>::commit(const Object &object,
                                const std::string &name) const {
  detail::h5t_commit2(object.getId(), name.c_str(), getId(), H5P_DEFAULT,
                      H5P_DEFAULT, H5P_DEFAULT);
}
...
template <typename T> inline DataType create_datatype() {
  return AtomicType<T>();
}

/// \brief Create a DataType instance representing type T and perform a sanity
/// check on its size
template <typename T> inline DataType create_and_check_datatype() {
  ...
  if (sizeof(T) != t.getSize()) {
    ...
    ss << "Size of array type " << sizeof(T) << " != that of memory datatype "
       << t.getSize() << std::endl;
...
HIGHFIVE_REGISTER_TYPE(HighFive::details::Boolean,
                       HighFive::create_enum_boolean)

namespace HighFive {

template <> inline DataType create_datatype<bool>() {
