...
namespace detail {
inline DataSpace make_data_space(hid_t hid) { return DataSpace::fromId(hid); }
} // namespace detail
...
inline DataSpace::DataSpace(const IT begin, const IT end) {
  ...
  _hid = detail::h5s_create_simple(int(real_dims.size()), real_dims.data(),
                                   nullptr);
}
...
inline DataSpace::DataSpace(const std::vector<size_t> &dims,
  ...
  _hid = detail::h5s_create_simple(int(dims.size()), real_dims.data(),
                                   real_maxdims.data());
}
...
template <typename T> inline DataSpace DataSpace::From(const T &value) {
...
inline bool checkDimensions(const DataSpace &mem_space,
  ...
  return checkDimensions(mem_space.getDimensions(), min_dim_requested,
                         max_dim_requested);
}
