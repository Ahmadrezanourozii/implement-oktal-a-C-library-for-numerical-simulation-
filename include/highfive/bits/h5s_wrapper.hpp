...
inline hid_t h5s_create_simple(int rank, const hsize_t dims[],
...
inline herr_t h5s_select_none(hid_t spaceid) {
  ...
  if (err < 0) {
    HDF5ErrMapper::ToException<DataSpaceException>(
        "Unable to select None space");
  }
  return err;
}

inline herr_t h5s_select_hyperslab(hid_t space_id, H5S_seloper_t op,
                                   const hsize_t start[],
...
inline herr_t h5s_select_elements(hid_t space_id, H5S_seloper_t op,
...
inline herr_t h5s_get_simple_extent_dims(hid_t space_id, hsize_t dims[],
                                         hsize_t maxdims[]) {
  herr_t err = H5Sget_simple_extent_dims(space_id, dims, maxdims);
  if (err < 0) {
    HDF5ErrMapper::ToException<DataSetException>(
        "Unable to get dimensions of dataspace");
  }
  return err;
}
...
inline H5S_class_t h5s_get_simple_extent_type(hid_t space_id) {
  ...
  if (cls == H5S_NO_CLASS) {
    HDF5ErrMapper::ToException<DataSpaceException>(
        "Unable to get class of simple dataspace.");
  }
...
inline H5S_sel_type h5s_get_select_type(hid_t space_id) {
  ...
  if (type < 0) {
    HDF5ErrMapper::ToException<DataSpaceException>(
        "Unable to get type of selection.");
  }
...
inline hid_t h5s_combine_select(hid_t space1_id, H5S_seloper_t op,
                                hid_t space2_id) {
  auto space_id = H5Scombine_select(space1_id, op, space2_id);
  if (space_id == H5I_INVALID_HID) {
    HDF5ErrMapper::ToException<DataSpaceException>(
        "Unable to combine two selections.");
  }
