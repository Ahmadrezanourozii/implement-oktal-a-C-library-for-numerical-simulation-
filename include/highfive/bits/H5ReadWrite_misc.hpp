...
template <typename T> struct type_char_array {
  using type = typename std::conditional<
      std::is_same<typename inspector<T>::base_type, std::string>::value,
      std::string, void>::type;
  static constexpr bool is_char_array = false;
};

template <typename T> struct type_char_array<T *> {
  using type =
      typename std::conditional<std::is_same<unqualified_t<T>, char>::value,
                                char *,
                                typename type_char_array<T>::type>::type;
  static constexpr bool is_char_array = true;
};

template <typename T, std::size_t N> struct type_char_array<T[N]> {
  using type =
      typename std::conditional<std::is_same<unqualified_t<T>, char>::value,
                                char[N],
                                typename type_char_array<T>::type>::type;
...
template <typename T> struct BufferInfo {
  ...
  static constexpr bool is_char_array =
      details::type_char_array<type_no_const>::is_char_array;
...
template <typename SrcStrT> struct string_type_checker {
...
inline void enforce_ascii_hack(const DataType &dst, const DataType &src) {
  ...
  // TEMP. CHANGE: Ensure that the character set is properly configured to
  // prevent converter issues on HDF5 <=v1.12.0 when loading ASCII strings
  // first. See https://github.com/HDFGroup/hdf5/issues/544 for further
  // information.
...
template <> struct string_type_checker<void> {
  inline static DataType getDataType(const DataType &element_type,
...
template <> struct string_type_checker<std::string> {
  inline static DataType getDataType(const DataType &,
...
template <std::size_t FixedLen> struct string_type_checker<char[FixedLen]> {
  inline static DataType getDataType(const DataType &element_type,
                                     const DataType &dtype) {
    DataType return_type =
        (dtype.isFixedLenStr()) ? AtomicType<char[FixedLen]>() : element_type;
    enforce_ascii_hack(return_type, dtype);
    return return_type;
  }
};

template <> struct string_type_checker<char *> {
...
template <typename T>
template <class F>
BufferInfo<T>::BufferInfo(const DataType &file_data_type, F getName,
                          Operation _op)
    : op(_op), is_fixed_len_string(file_data_type.isFixedLenStr())
      // In case we are using Fixed-len strings we need to subtract one
      // dimension
      ,
      data_type(string_type_checker<char_array_t>::getDataType(
          create_datatype<elem_type>(), file_data_type)),
      rank_correction((is_fixed_len_string && is_char_array) ? 1 : 0) {
  // We warn. In case they are really not convertible an exception will rise on
  // read/write
  if (file_data_type.getClass() != data_type.getClass()) {
    HIGHFIVE_LOG_WARN(getName() +
                      "\": data and hdf5 dataset have different types: " +
                      data_type.string() + " -> " + file_data_type.string());
  } else if ((file_data_type.getClass() & data_type.getClass()) ==
             DataTypeClass::Float) {
    HIGHFIVE_LOG_WARN_IF((op == Operation::read) &&
                             (file_data_type.getSize() > data_type.getSize()),
                         getName() +
                             "\": hdf5 dataset has higher floating point "
                             "precision than data on read: " +
                             file_data_type.string() + " -> " +
                             data_type.string());

    HIGHFIVE_LOG_WARN_IF((op == Operation::write) &&
                             (file_data_type.getSize() < data_type.getSize()),
                         getName() +
                             "\": data has higher floating point precision "
                             "than hdf5 dataset on write: " +
                             data_type.string() + " -> " +
                             file_data_type.string());
  }
}

template <typename T> size_t BufferInfo<T>::getRank(const T &array) const {
  return details::inspector<type_no_const>::getRank(array) - rank_correction;
}

template <typename T> size_t BufferInfo<T>::getMinRank() const {
  return details::inspector<T>::min_ndim - rank_correction;
}

template <typename T> size_t BufferInfo<T>::getMaxRank() const {
