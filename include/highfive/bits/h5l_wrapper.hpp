...
inline herr_t h5l_create_external(const char *file_name, const char *obj_name,
                                  hid_t link_loc_id, const char *link_name,
                                  hid_t lcpl_id, hid_t lapl_id) {
  herr_t err = H5Lcreate_external(file_name, obj_name, link_loc_id, link_name,
                                  lcpl_id, lapl_id);
  if (err < 0) {
...
inline herr_t h5l_create_soft(const char *link_target, hid_t link_loc_id,
                              const char *link_name, hid_t lcpl_id,
                              hid_t lapl_id) {
  herr_t err =
      H5Lcreate_soft(link_target, link_loc_id, link_name, lcpl_id, lapl_id);
  if (err < 0) {
    HDF5ErrMapper::ToException<GroupException>(
...
inline herr_t h5l_create_hard(hid_t cur_loc, const char *cur_name,
                              hid_t dst_loc, const char *dst_name,
                              hid_t lcpl_id, hid_t lapl_id) {
  herr_t err =
      H5Lcreate_hard(cur_loc, cur_name, dst_loc, dst_name, lcpl_id, lapl_id);
  if (err < 0) {
    HDF5ErrMapper::ToException<GroupException>(
        std::string("Unable to create hard link: "));
  }

  return err;
}

inline herr_t h5l_get_info(hid_t loc_id, const char *name, H5L_info_t *linfo,
                           hid_t lapl_id) {
  herr_t err = H5Lget_info(loc_id, name, linfo, lapl_id);
  if (err < 0) {
    HDF5ErrMapper::ToException<GroupException>(
        std::string("Unable to obtain info for link "));
  }
...
inline herr_t h5l_iterate(hid_t grp_id, H5_index_t idx_type,
...
inline herr_t h5l_move(hid_t src_loc, const char *src_name, hid_t dst_loc,
                       const char *dst_name, hid_t lcpl_id, hid_t lapl_id) {
  herr_t err = H5Lmove(src_loc, src_name, dst_loc, dst_name, lcpl_id, lapl_id);

  if (err < 0) {
    HDF5ErrMapper::ToException<GroupException>(
        std::string("Unable to move link to \"") + dst_name + "\":");
  }
  return err;
}

inline ssize_t h5l_get_name_by_idx(hid_t loc_id, const char *group_name,
                                   H5_index_t idx_type, H5_iter_order_t order,
                                   hsize_t n, char *name, size_t size,
                                   hid_t lapl_id) {
  ssize_t n_chars = H5Lget_name_by_idx(loc_id, group_name, idx_type, order, n,
                                       name, size, lapl_id);
