...
inline DataSet NodeTraits<Derivate>::createDataSet(
    const std::string &dataset_name, const DataSpace &space,
    const DataType &dtype, const DataSetCreateProps &createProps,
    const DataSetAccessProps &accessProps, bool parents) {
  ...
  return DataSet(detail::h5d_create2(static_cast<Derivate *>(this)->getId(),
                                     dataset_name.c_str(), dtype.getId(),
                                     space.getId(), lcpl.getId(),
...
inline DataSet NodeTraits<Derivate>::createDataSet(
    ...
    const DataSetAccessProps &accessProps, bool parents) {
  return createDataSet(dataset_name, space, create_and_check_datatype<T>(),
...
template <typename T>
inline DataSet NodeTraits<Derivate>::createDataSet(
    const std::string &dataset_name, const T &data,
    const DataSetCreateProps &createProps,
    const DataSetAccessProps &accessProps, bool parents) {
...
inline Group NodeTraits<Derivate>::createGroup(const std::string &group_name,
  ...
  return detail::make_group(detail::h5g_create2(
      static_cast<Derivate *>(this)->getId(), group_name.c_str(), lcpl.getId(),
      H5P_DEFAULT, H5P_DEFAULT));
}

template <typename Derivate>
inline Group
NodeTraits<Derivate>::createGroup(const std::string &group_name,
...
NodeTraits<Derivate>::getGroup(const std::string &group_name) const {
  return detail::make_group(
      detail::h5g_open2(static_cast<const Derivate *>(this)->getId(),
...
inline DataType NodeTraits<Derivate>::getDataType(
  ...
  return DataType(
      detail::h5t_open2(static_cast<const Derivate *>(this)->getId(),
                        type_name.c_str(), accessProps.getId()));
}
...
inline std::string NodeTraits<Derivate>::getObjectName(size_t index) const {
  return details::get_name([&](char *buffer, size_t length) {
    return detail::h5l_get_name_by_idx(
        ...
        H5_ITER_INC, index, buffer, length, H5P_DEFAULT);
  });
}
...
inline bool NodeTraits<Derivate>::rename(const std::string &src_path,
  ...
  herr_t err = detail::h5l_move(static_cast<const Derivate *>(this)->getId(),
                                ...
                                dst_path.c_str(), lcpl.getId(), H5P_DEFAULT);
...
NodeTraits<Derivate>::listObjectNames(IndexType idx_type) const {
  ...
  detail::h5l_iterate(static_cast<const Derivate *>(this)->getId(),
                      static_cast<H5_index_t>(idx_type), H5_ITER_INC, NULL,
...
inline bool NodeTraits<Derivate>::_exist(const std::string &node_name,
  ...
  const auto val =
      detail::nothrow::h5l_exists(static_cast<const Derivate *>(this)->getId(),
...
inline void NodeTraits<Derivate>::unlink(const std::string &node_name) const {
  detail::h5l_delete(static_cast<const Derivate *>(this)->getId(),
                     node_name.c_str(), H5P_DEFAULT);
}

// convert internal link types to enum class.
// This function is internal, so H5L_TYPE_ERROR shall be handled in the calling
// context
...
template <typename Derivate>
inline LinkType
NodeTraits<Derivate>::getLinkType(const std::string &node_name) const {
  H5L_info_t linkinfo;
  detail::h5l_get_info(static_cast<const Derivate *>(this)->getId(),
                       node_name.c_str(), &linkinfo, H5P_DEFAULT);

  if (linkinfo.type == H5L_TYPE_ERROR) {
    HDF5ErrMapper::ToException<GroupException>(
        std::string("Link type of \"") + node_name + "\" is H5L_TYPE_ERROR");
  }
...
template <typename Derivate>
inline ObjectType
NodeTraits<Derivate>::getObjectType(const std::string &node_name) const {
...
inline void NodeTraits<Derivate>::createSoftLink(
    const std::string &link_name, const std::string &obj_path,
    LinkCreateProps linkCreateProps, const LinkAccessProps &linkAccessProps,
    const bool parents) {
  if (parents) {
    linkCreateProps.add(CreateIntermediateGroup{});
  }
  detail::h5l_create_soft(
      obj_path.c_str(), static_cast<const Derivate *>(this)->getId(),
      link_name.c_str(), linkCreateProps.getId(), linkAccessProps.getId());
}

template <typename Derivate>
inline void NodeTraits<Derivate>::createExternalLink(
    const std::string &link_name, const std::string &h5_file,
    const std::string &obj_path, LinkCreateProps linkCreateProps,
    const LinkAccessProps &linkAccessProps, const bool parents) {
  if (parents) {
    linkCreateProps.add(CreateIntermediateGroup{});
  }
  detail::h5l_create_external(h5_file.c_str(), obj_path.c_str(),
                              static_cast<const Derivate *>(this)->getId(),
                              link_name.c_str(), linkCreateProps.getId(),
...
inline void NodeTraits<Derivate>::createHardLink(
    const std::string &link_name, const T &target_obj,
    LinkCreateProps linkCreateProps, const LinkAccessProps &linkAccessProps,
    const bool parents) {
  ...
  detail::h5l_create_hard(
      target_obj.getId(), ".", static_cast<const Derivate *>(this)->getId(),
      link_name.c_str(), linkCreateProps.getId(), linkAccessProps.getId());
}
