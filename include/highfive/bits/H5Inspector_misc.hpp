...
inspector<T> {
    ...
    using base_type
    // hdf5_type is the base read by hdf5 (c-type) (e.g.
std::vector<std::string> => const char*) using hdf5_type
    ...
    // Take a serialized vector 'in', some dims and copy value to val (for
reading) static void unserialize(const hdf5_type* in, const
std::vector<size_t>&i, type& val)
    ...
    static void serialize(const type& val, const std::vector<size_t>& dims,
hdf5_type* out)
...
template <typename T> struct inspector : type_helper<T> {};
...
template <> struct inspector<bool> : type_helper<bool> {
...
template <> struct inspector<std::string> : type_helper<std::string> {
  ...
  template <class It>
  static void unserialize(const It &vec, const std::vector<size_t> & /* dims */,
                          type &val) {
    const auto &view = *vec;
...
template <> struct inspector<Reference> : type_helper<Reference> {
...
template <typename T> struct inspector<std::vector<T>> {
  ...
  template <class It>
  static void unserialize(const It &vec_align, const std::vector<size_t> &dims,
                          type &val) {
    std::vector<size_t> next_dims(dims.begin() + 1, dims.end());
    size_t next_size = compute_total_size(next_dims);
    for (size_t i = 0; i < dims[0]; ++i) {
      inspector<value_type>::unserialize(vec_align + i * next_size, next_dims,
                                         val[i]);
    }
  }
};

template <> struct inspector<std::vector<bool>> {
...
template <typename T, size_t N> struct inspector<std::array<T, N>> {
  ...
  static constexpr bool is_trivially_copyable =
      ...
      inspector<value_type>::is_trivially_nestable;
  static constexpr bool is_trivially_nestable =
      (sizeof(type) == N * sizeof(T)) && is_trivially_copyable;
  ...
  static void prepare(type &val, const std::vector<size_t> &dims) {
    if (dims[0] > N) {
      ...
      os << "Size of std::array (" << N << ") is too small for dims ("
         << dims[0] << ").";
  ...
  static void unserialize(const It &vec_align, const std::vector<size_t> &dims,
    ...
    if (dims[0] != N) {
      std::ostringstream os;
      os << "Impossible to pair DataSet with " << dims[0]
         << " elements into an array with " << N << " elements.";
      throw DataSpaceException(os.str());
    }
    std::vector<size_t> next_dims(dims.begin() + 1, dims.end());
    size_t next_size = compute_total_size(next_dims);
...
template <typename T> struct inspector<T *> {
  using type = T *;
  using value_type = unqualified_t<T>;
  using base_type = typename inspector<value_type>::base_type;
  using hdf5_type = typename inspector<value_type>::hdf5_type;
...
// Cannot be use for reading
template <typename T, size_t N> struct inspector<T[N]> {
  using type = T[N];
  using value_type = unqualified_t<T>;
  using base_type = typename inspector<value_type>::base_type;
  using hdf5_type = typename inspector<value_type>::hdf5_type;
