...
inline hid_t h5a_create2(hid_t loc_id, char const *const attr_name,
                         hid_t type_id, hid_t space_id, hid_t acpl_id,
                         hid_t aapl_id) {
  auto attr_id =
      H5Acreate2(loc_id, attr_name, type_id, space_id, acpl_id, aapl_id);
  if (attr_id < 0) {
...
inline hid_t h5a_open(hid_t loc_id, char const *const attr_name,
                      hid_t aapl_id) {
...
inline herr_t h5a_read(hid_t attr_id, hid_t type_id, void *buf) {
  ...
  if (err < 0) {
    HDF5ErrMapper::ToException<AttributeException>(
        std::string("Unable to read attribute"));
  }
...
inline herr_t h5a_write(hid_t attr_id, hid_t type_id, void const *buf) {
  ...
  if (err < 0) {
    HDF5ErrMapper::ToException<AttributeException>(
        std::string("Unable to write attribute"));
  }
