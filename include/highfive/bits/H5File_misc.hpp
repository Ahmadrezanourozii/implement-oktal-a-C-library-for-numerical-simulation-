...
inline File::File(const std::string &filename, AccessMode openFlags,
                  const FileAccessProps &fileAccessProps)
    : File(filename, openFlags, FileCreateProps::Default(), fileAccessProps) {}

inline File::File(const std::string &filename, AccessMode access_mode,
  ...
  if (!mustCreate) {
    ...
    _hid = detail::nothrow::h5f_open(filename.c_str(), openMode,
...
inline hsize_t File::getFileSpacePageSize() const {
  ...
  if (getFileSpaceStrategy() != H5F_FSPACE_STRATEGY_PAGE) {
    HDF5ErrMapper::ToException<FileException>(std::string(
...
inline void File::flush() { detail::h5f_flush(_hid, H5F_SCOPE_GLOBAL); }
