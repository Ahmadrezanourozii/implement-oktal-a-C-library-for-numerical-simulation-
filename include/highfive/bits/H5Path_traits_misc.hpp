...
template <typename Derivate> inline PathTraits<Derivate>::PathTraits() {
  static_assert(
      std::is_same<Derivate, Group>::value ||
  ...
  if (obj.isValid()) {
    const hid_t file_id =
        detail::h5i_get_file_id<PropertyException>(obj.getId());
...
inline std::string PathTraits<Derivate>::getPath() const {
  return details::get_name([this](char *buffer, size_t length) {
    return detail::h5i_get_name(static_cast<const Derivate &>(*this).getId(),
                                buffer, length);
  });
}
