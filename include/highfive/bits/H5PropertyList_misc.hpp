...
inline PropertyListBase::PropertyListBase() noexcept : Object(H5P_DEFAULT) {}

template <PropertyType T> inline void PropertyList<T>::_initializeIfNeeded() {
  if (_hid != H5P_DEFAULT) {
    return;
  }
...
inline FileSpaceStrategy::FileSpaceStrategy(H5F_fspace_strategy_t strategy,
    ...
    : _strategy(strategy), _persist(persist), _threshold(threshold) {}

inline FileSpaceStrategy::FileSpaceStrategy(const FileCreateProps &fcpl) {
  detail::h5p_get_file_space_strategy(fcpl.getId(), &_strategy, &_persist,
                                      &_threshold);
}
...
inline hbool_t FileSpaceStrategy::getPersist() const { return _persist; }

inline hsize_t FileSpaceStrategy::getThreshold() const { return _threshold; }
...
inline unsigned PageBufferSize::getMinMetaPercent() const { return _min_meta; }

inline unsigned PageBufferSize::getMinRawPercent() const { return _min_raw; }
#endif
#endif
...
inline MPIOCollectiveMetadata::MPIOCollectiveMetadata(bool collective)
    : collective_read_(collective), collective_write_(collective) {}

inline MPIOCollectiveMetadata::MPIOCollectiveMetadata(
...
inline MPIOCollectiveMetadataRead::MPIOCollectiveMetadataRead(
    const FileAccessProps &plist) {
...
inline MPIOCollectiveMetadataWrite::MPIOCollectiveMetadataWrite(
    const FileAccessProps &plist) {
...
inline std::pair<H5F_libver_t, H5F_libver_t>
FileVersionBounds::getVersion() const {
...
inline MetadataBlockSize::MetadataBlockSize(hsize_t size) : _size(size) {}
...
inline hsize_t MetadataBlockSize::getSize() const { return _size; }
...
inline EstimatedLinkInfo::EstimatedLinkInfo(unsigned entries, unsigned length)
    : _entries(entries), _length(length) {}
...
inline unsigned EstimatedLinkInfo::getEntries() const { return _entries; }

inline unsigned EstimatedLinkInfo::getNameLength() const { return _length; }

inline void Chunking::apply(const hid_t hid) const {
  detail::h5p_set_chunk(hid, static_cast<int>(_dims.size()), _dims.data());
}

inline Chunking::Chunking(const std::vector<hsize_t> &dims) : _dims(dims) {}
...
inline Chunking::Chunking(DataSetCreateProps &plist, size_t max_dims)
    : _dims(max_dims + 1) {
  auto n_loaded = detail::h5p_get_chunk(
      plist.getId(), static_cast<int>(_dims.size()), _dims.data());
...
inline Deflate::Deflate(unsigned int level) : _level(level) {}
...
inline Szip::Szip(unsigned int options_mask, unsigned int pixels_per_block)
    : _options_mask(options_mask), _pixels_per_block(pixels_per_block) {}

inline unsigned Szip::getOptionsMask() const { return _options_mask; }

inline unsigned Szip::getPixelsPerBlock() const { return _pixels_per_block; }
...
inline Caching::Caching(const size_t numSlots, const size_t cacheSize,
                        const double w0)
    : _numSlots(numSlots), _cacheSize(cacheSize), _w0(w0) {}

inline size_t Caching::getNumSlots() const { return _numSlots; }

inline size_t Caching::getCacheSize() const { return _cacheSize; }

inline double Caching::getW0() const { return _w0; }
...
inline CreateIntermediateGroup::CreateIntermediateGroup(
    const LinkCreateProps &lcpl) {
...
inline bool CreateIntermediateGroup::isSet() const { return _create; }

#ifdef H5_HAVE_PARALLEL
inline UseCollectiveIO::UseCollectiveIO(bool enable) : _enable(enable) {}

inline void UseCollectiveIO::apply(const hid_t hid) const {
  detail::h5p_set_dxpl_mpio(hid, _enable ? H5FD_MPIO_COLLECTIVE
...
inline UseCollectiveIO::UseCollectiveIO(const DataTransferProps &dxpl) {
  ...
  if (collective != H5FD_MPIO_COLLECTIVE &&
      collective != H5FD_MPIO_INDEPENDENT) {
...
inline bool UseCollectiveIO::isCollective() const { return _enable; }

inline MpioNoCollectiveCause::MpioNoCollectiveCause(
    const DataTransferProps &dxpl) {
  detail::h5p_get_mpio_no_collective_cause(dxpl.getId(), &_local_cause,
                                           &_global_cause);
}
...
inline unsigned LinkCreationOrder::getFlags() const { return _flags; }
...
inline AttributePhaseChange::AttributePhaseChange(unsigned max_compact,
                                                  unsigned min_dense)
    : _max_compact(max_compact), _min_dense(min_dense) {}

inline AttributePhaseChange::AttributePhaseChange(
    const GroupCreateProps &gcpl) {
...
inline unsigned AttributePhaseChange::min_dense() const { return _min_dense; }
