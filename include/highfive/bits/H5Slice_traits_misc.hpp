...
inline const DataSet &get_dataset(const DataSet &ds) { return ds; }
...
inline hid_t get_memspace_id(const DataSet &) { return H5S_ALL; }
} // namespace details
...
class HyperCube {
public:
  HyperCube(size_t rank) : offset(rank), count(rank) {}
...
inline void build_hyper_slab(HyperSlab &slab, size_t /* axis */,
                             HyperCube &cube) {
  slab |= cube.asSlab();
}
...
template <class... Slices>
inline void build_hyper_slab(HyperSlab &slab, size_t axis, HyperCube &cube,
                             size_t id, const Slices &...higher_slices) {
  auto slice = std::array<size_t, 2>{id, id + 1};
  build_hyper_slab(slab, axis, cube, slice, higher_slices...);
}
...
template <class... Slices>
inline void
compute_squashed_shape(size_t axis, std::vector<size_t> &shape,
                       const std::vector<std::array<size_t, 2>> &slices,
                       const Slices &...higher_slices);
...
inline Selection
SliceTraits<Derivate>::select(const HyperSlab &hyperslab,
  ...
  return detail::make_selection(memspace, filespace,
                                details::get_dataset(slice));
}

template <typename Derivate>
inline Selection
SliceTraits<Derivate>::select(const HyperSlab &hyper_slab) const {
  const auto &slice = static_cast<const Derivate &>(*this);
...
inline Selection SliceTraits<Derivate>::select(
    const std::vector<size_t> &offset, const std::vector<size_t> &count,
    const std::vector<size_t> &stride, const std::vector<size_t> &block) const {
...
SliceTraits<Derivate>::select(const ElementSet &elements) const {
  ...
  detail::h5s_select_elements(space.getId(), H5S_SELECT_SET, num_elements,
                              data);

  return detail::make_selection(DataSpace(num_elements), space,
                                details::get_dataset(slice));
}

template <typename Derivate>
inline Selection
SliceTraits<Derivate>::select(const ProductSet &product_set) const {
...
template <typename T>
inline T
SliceTraits<Derivate>::read(const DataTransferProps &xfer_props) const {
  T array;
...
template <typename T>
inline void
SliceTraits<Derivate>::read(T &array,
  ...
  if (!details::checkDimensions(mem_space, buffer_info.getMinRank(),
                                buffer_info.getMaxRank())) {
    ...
    ss << "Impossible to read DataSet of dimensions "
       ...
       << "(min) to " << buffer_info.getMaxRank() << "(max)";
  ...
  if (c == DataTypeClass::VarLen || t.isVariableStr()) {
    ...
    (void)detail::h5t_reclaim(t.getId(), mem_space.getId(), xfer_props.getId(),
                              r.getPointer());
#else
    // This one is deprecated since 1.12.0
    (void)detail::h5d_vlen_reclaim(t.getId(), mem_space.getId(),
                                   xfer_props.getId(), r.getPointer());
#endif
  }
}
...
template <typename T>
inline void
SliceTraits<Derivate>::read_raw(T *array, const DataType &mem_datatype,
                                const DataTransferProps &xfer_props) const {
  ...
  detail::h5d_read(details::get_dataset(slice).getId(), mem_datatype.getId(),
                   details::get_memspace_id(slice), slice.getSpace().getId(),
                   xfer_props.getId(), static_cast<void *>(array));
}

template <typename Derivate>
template <typename T>
inline void
SliceTraits<Derivate>::read_raw(T *array,
                                const DataTransferProps &xfer_props) const {
...
inline void SliceTraits<Derivate>::write(const T &buffer,
  ...
  if (!details::checkDimensions(mem_space, buffer_info.getMinRank(),
                                buffer_info.getMaxRank())) {
    std::ostringstream ss;
    ss << "Impossible to write buffer with dimensions n = "
       << buffer_info.getRank(buffer) << "into dataset with dimensions "
...
template <typename T>
inline void
SliceTraits<Derivate>::write_raw(const T *buffer, const DataType &mem_datatype,
                                 const DataTransferProps &xfer_props) {
  const auto &slice = static_cast<const Derivate &>(*this);

  detail::h5d_write(details::get_dataset(slice).getId(), mem_datatype.getId(),
                    details::get_memspace_id(slice), slice.getSpace().getId(),
                    xfer_props.getId(), static_cast<const void *>(buffer));
}

template <typename Derivate>
template <typename T>
inline void
SliceTraits<Derivate>::write_raw(const T *buffer,
                                 const DataTransferProps &xfer_props) {
  using element_type = typename details::inspector<T>::base_type;
...
inline const DataSet &getDataSet(const DataSet &dataset) { return dataset; }

} // namespace detail

template <typename Derivate>
inline Selection
SliceTraits<Derivate>::squeezeMemSpace(const std::vector<size_t> &axes) const {
  ...
  return detail::make_selection(DataSpace(squeezed_dims), slice.getSpace(),
                                detail::getDataSet(slice));
}

template <typename Derivate>
inline Selection SliceTraits<Derivate>::reshapeMemSpace(
    const std::vector<size_t> &new_dims) const {
  auto slice = static_cast<const Derivate &>(*this);

  detail::assert_compatible_spaces(slice.getMemSpace(), new_dims);
  return detail::make_selection(DataSpace(new_dims), slice.getSpace(),
                                detail::getDataSet(slice));
}
