...
public:
  ...
  /// \brief Create a list of points of N-dimension for selection.
  ///
  /// \param element_ids List of continuous coordinates (e.g.: in 2 dimensions
  /// space `ElementSet{1, 2, 3 ,4}` creates points `(1, 2)` and `(3, 4)`).
...
struct RegularHyperSlab {
  ...
  RegularHyperSlab(const std::vector<size_t> &offset_,
      ...
      : offset(toHDF5SizeVector(offset_)), count(toHDF5SizeVector(count_)),
        stride(toHDF5SizeVector(stride_)), block(toHDF5SizeVector(block_)) {}
...
protected:
  ...
  size_t detect_streak(Select_ const *begin, Select_ const *end, Op op) const {
    ...
    auto it = std::find_if(begin, end,
                           [op](const Select_ &sel) { return sel.op != op; });
    return static_cast<size_t>(it - begin);
  }

  DataSpace combine_selections(const DataSpace &left_space, Op op,
    ...
    if (left_type == H5S_SEL_NONE) {
      ...
      return detail::make_data_space(detail::h5s_combine_select(
          left_space.getId(), convert(op), right_space.getId()));
    }
  }
  ...
  DataSpace reduce_streak(const DataSpace &outer_space, Select_ const *begin,
                          Select_ const *end, Op op) const {
    assert(op == Op::Or);
...
class ProductSet {
public:
  template <class... Slices> explicit ProductSet(const Slices &...slices);
...
public:
  ...
  ///
  /// HyperSlabs can be either regular or irregular. Irregular hyperslabs are
  /// typically generated by taking the union of regular hyperslabs. An
  /// irregular hyperslab, in general, does not fit nicely into a
  /// multi-dimensional array, but only a subset of such an array.
  ///
  /// Therefore, the only memspaces supported for general hyperslabs are
  ...
  ///
  /// \brief Select a region in the current Slice/Dataset out of a list of
  /// elements.
  ///
  ...
  /// \param dtype: The type of the data, in case it cannot be automatically
  /// guessed
  /// \param xfer_props: Data Transfer properties
  template <typename T>
  void
  read_raw(T *array, const DataType &dtype,
           const DataTransferProps &xfer_props = DataTransferProps()) const;
  ...
  /// \param xfer_props: The HDF5 data transfer properties, e.g. collective
  /// MPI-IO.
  template <typename T>
