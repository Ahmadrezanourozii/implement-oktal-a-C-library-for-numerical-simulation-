...
inline hid_t h5f_create(const char *filename, unsigned flags, hid_t fcpl_id,
                        hid_t fapl_id) {
...
inline ssize_t h5f_get_name(hid_t obj_id, char *name, size_t size) {
  ...
  if (nread < 0) {
    HDF5ErrMapper::ToException<FileException>(
        std::string("Failed to get file from id"));
  }
...
inline herr_t h5f_flush(hid_t object_id, H5F_scope_t scope) {
  ...
  if (err < 0) {
    HDF5ErrMapper::ToException<FileException>(
        std::string("Failed to flush file"));
  }
...
inline herr_t h5f_get_filesize(hid_t file_id, hsize_t *size) {
  ...
  if (err < 0) {
    HDF5ErrMapper::ToException<FileException>(
        std::string("Unable to retrieve size of file"));
  }
