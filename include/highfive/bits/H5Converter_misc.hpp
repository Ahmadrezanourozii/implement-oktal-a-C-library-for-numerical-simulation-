...
template <class T> struct is_std_string {
...
struct enable_shallow_copy
    : public std::enable_if<
          !is_std_string<T>::value && inspector<T>::is_trivially_copyable, V> {
};
...
template <typename T, bool IsReadOnly> struct ShallowCopyBuffer {
...
///
inline size_t char_buffer_length(char const *const str,
...
/// different input depending on whether the strings are fixed or variable
/// length. For fixed length strings, it expects an array of chars, i.e. one
/// string packed after the other contiguously. While for variable length
/// strings it expects a list of pointers to the beginning of each string.
/// Variable length string must be null-terminated; because that's how their
...
template <typename T, BufferMode buffer_mode> struct StringBuffer {
  using type = unqualified_t<T>;
  using hdf5_type = typename inspector<type>::hdf5_type;
    ...
    void assign(char const *data, size_t length, StringPadding pad) {
      if (buffer.isVariableLengthString()) {
        if (pad == StringPadding::NullTerminated) {
          ...
          buffer.variable_length_pointers[i] =
              buffer.variable_length_buffer[i].data();
        }
        ...
        memcpy(&buffer.fixed_length_buffer[i * buffer.string_size], data,
               length);
      }
    }
  ...
  StringBuffer(std::vector<size_t> _dims, const DataType &_file_datatype)
      : file_datatype(_file_datatype.asStringType()),
        ...
        string_size(file_datatype.isVariableStr() ? size_t(-1)
                                                  : file_datatype.getSize()),
...
private:
  ...
  std::vector<typename std::conditional<buffer_mode == BufferMode::Write,
...
template <typename T>
struct Writer<T, typename enable_shallow_copy<T>::type>
    : public ShallowCopyBuffer<T, true> {
private:
  using super = ShallowCopyBuffer<T, true>;

public:
  explicit Writer(const T &val, const std::vector<size_t> & /* dims */,
                  const DataType & /* file_datatype */)
      : super(val) {};
};

template <typename T>
struct Writer<T, typename enable_deep_copy<T>::type>
    : public DeepCopyBuffer<T> {
  explicit Writer(const T &val, const std::vector<size_t> &_dims,
                  const DataType & /* file_datatype */)
...
template <typename T>
struct Writer<T, typename enable_string_copy<T>::type>
...
template <typename T, typename Enable = void> struct Reader;

template <typename T>
struct Reader<T, typename enable_shallow_copy<T>::type>
...
template <typename T>
struct Reader<T, typename enable_deep_copy<T>::type>
    : public DeepCopyBuffer<T> {
private:
...
template <typename T>
struct Reader<T, typename enable_string_copy<T>::type>
    : public StringBuffer<T, BufferMode::Write> {
public:
  explicit Reader(const std::vector<size_t> &_dims, const T & /* val */,
                  const DataType &_file_datatype)
      : StringBuffer<T, BufferMode::Write>(_dims, _file_datatype) {}
};
