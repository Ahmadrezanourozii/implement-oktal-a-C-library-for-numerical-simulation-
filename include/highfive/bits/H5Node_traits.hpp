...
public:
  ...
  DataSet getDataSet(const std::string &dataset_name,
                     const DataSetAccessProps &accessProps =
                         DataSetAccessProps::Default()) const;
  ...
  Group createGroup(const std::string &group_name,
                    const GroupCreateProps &createProps, bool parents = true);
  ...
  DataType getDataType(const std::string &type_name,
                       const DataTypeAccessProps &accessProps =
                           DataTypeAccessProps::Default()) const;
  ...
  bool rename(const std::string &src_path, const std::string &dest_path,
              bool parents = true) const;

  ///
  /// \brief list all leaf objects name of the node / group
  /// \param idx_type tell if the list should be ordered by Name or
  /// CreationOrderTime. CreationOrderTime can be use only if the file/group has
  /// been created with the HighFive::LinkCreationTime property.
  /// \return number of leaf objects
  std::vector<std::string>
  listObjectNames(IndexType idx_type = IndexType::NAME) const;
  ...
  ///
  /// \brief A shorthand to get the kind of object pointed to (group, dataset,
  /// type...)
  /// \param node_name The entry to check, path relative to the current group
  ObjectType getObjectType(const std::string &node_name) const;

  ///
  /// \brief A shorthand to create softlink to any object which provides
  /// `getPath` The link will be created with default properties along with
  ...
  /// \param linkCreateProps A Link_Create property list. Notice "parents=true"
  /// overrides
  /// \param linkAccessProps The Link_Access property list
  /// \param parents Whether parent groups should be created: Default: true
  void
  createSoftLink(const std::string &link_name, const std::string &obj_path,
  ...
  void
  createExternalLink(const std::string &link_name, const std::string &h5_file,
  ...
  template <typename T, typename = decltype(&T::getPath)>
  void
  createHardLink(const std::string &link_name, const T &target_obj,
                 LinkCreateProps linkCreateProps = LinkCreateProps(),
                 const LinkAccessProps &linkAccessProps = LinkAccessProps(),
                 const bool parents = true);
